<template>
  <div class="flex">
    <div class="select">
      <select :name="name" v-if="stores.value.length">
        <option value="" disabled selected>Select a store</option>
        <option v-for="store in stores.value" :value="store.id" :selected="store.id == value">{{ store.label }}</option>
      </select>
    </div>
  ...
  <div v-if="showingStoreSetup">
    <div class="pane">
      <div class="field">
        ...
        <input class="text fullwidth" v-model="newStore.id" type="text" id="newStoreId"/>
      </div>
      <div class="field">
        <div class="heading">
          <label for="newStoreLabel">Store Label</label>
        </div>
        <input class="text fullwidth" v-model="newStore.label" type="text" id="newStoreLabel"/>
      </div>
      <div class="field">
        <div class="heading">
          <label for="newStoreUrl">Store Admin URL</label>
        </div>
          <input class="text fullwidth" v-model="newStore.adminUri" type="text" id="newStoreUrl"/>
      </div>
      <div v-if="error">
        <p>{{error}}</p>
      </div>
      <button class="btn submit" type="submit" @click.prevent="saveStore">Save</button>
    </div>
  </div>
</template>
